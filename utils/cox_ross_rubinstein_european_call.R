# Cox-Ross-Rubinstein binomial tree option pricing model / European Call Option
...
crr_european_call = function(current_price, strike_price, maturity, n_steps, volatility, risk_free_rate){
  ...
  # Payoffs at maturity
  for (j in 0:n_steps){
    tree[n_steps + 1, j + 1] = max(0, current_price * (u^j) * d^(n_steps - j) - strike_price)
  }
