# Kamrad-Ritchken trinomial tree option pricing model / American Put Option
...
kr_american_put = function(current_price, strike_price, maturity, n_steps, lambda, volatility, risk_free_rate){
  ...
  for (i in (ncol(option_tree) - 1):1){
    for (j in 1:(2 * (i - 1) + 1)){
      option_tree[j, i] = max(strike_price - stock_tree[j, i],
                                exp(-risk_free_rate * dt) * (p_down * option_tree[j, i + 1] + p_flat * option_tree[j + 1, i + 1] + p_up * option_tree[j + 2, i + 1]))
    }
  }
