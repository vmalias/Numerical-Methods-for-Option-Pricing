...
for (i in 1:n) {
 KR_1000steps[i] = KammradAmericanPut(So[i], 100, T[i], 1000, 1.22474, sigma[i], r[i])
  
  for (j in seq(1,length(Periods))){
    CRR[i,j] = BinTreeAmPut(So[i], 100, T[i], Periods[j], sigma[i], r[i])
    KR[i,j] = KammradAmericanPut(So[i], 100, T[i], Periods[j], 1.22474, sigma[i], r[i])
  }
}

#write.xlsx(KR_1000steps, "KR_1000steps.xlsx", colNames = FALSE, rowNames = FALSE)
#write.xlsx(CRR, "CRR.xlsx", colNames = TRUE, rowNames = FALSE)
#write.xlsx(KR, "KR.xlsx", colNames = TRUE, rowNames = FALSE)


y = which(KR_1000steps >= 0.5)

for (i in seq(1,length(Periods))) {
  RMSE_CRR[i] = sqrt(mean(((CRR[y,i] - KR_1000steps[y]) / KR_1000steps[y])^2))
  RMSE_KR[i] = sqrt(mean(((KR[y,i] - KR_1000steps[y]) / KR_1000steps[y])^2))
}
